...
int main(int argc, char** argv)
{
  ...
  //const Eigen::Isometry3d& pose1 = kinematic_state->getGlobalLinkTransform("cluedo_link");
  ...
  pose2.orientation.w = 0.70;
  pose2.orientation.x = -0.00;
  pose2.orientation.y = 0.00;
  pose2.orientation.z = -0.71;
  pose2.position.x =  1.00;
  pose2.position.y =  0.00;
  pose2.position.z =  1.25;
  
  group.setStartStateToCurrentState();
  group.setApproximateJointValueTarget(pose2, "cluedo_link");
  ...
  if (found_ik2)
  {
    ...
    for (std::size_t i = 0; i < joint_names.size(); ++i)
    {
      ROS_INFO("Joint %s: %f", joint_names[i].c_str(), joint_values2[i]);
    }
  }
  else
  {
  ...
  group.setJointValueTarget(joint_values2);
  group.setStartStateToCurrentState();
  group.setGoalOrientationTolerance(0.01);
  group.setGoalPositionTolerance(0.01);
...
/*
  ...
  std::cout << "Position 3 -> IK + setJointValue" << std::endl;
  sleep(5.0);
